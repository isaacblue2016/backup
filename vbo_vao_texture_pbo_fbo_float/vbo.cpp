...
int texture01 = 0;
float fTime = 0.0f;
static GLuint vbo_vertex = 101;
static GLuint vbo_coord = 102;
static GLuint vbo_indice = 103;
...
void renderTri()
{
    ...
    glVertexPointer(3, GL_FLOAT, 8 * sizeof(GLfloat), (GLvoid*)0);
    

    glGenBuffers(1, &vbo_coord);
    glBindBuffer(GL_ARRAY_BUFFER, vbo_coord);
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW);
    glTexCoordPointer(2, GL_FLOAT, 8 * sizeof(GLfloat), (GLvoid*)(6 * sizeof(GLfloat))); // (GLvoid*)(6 * sizeof(GLfloat)) is offset
